...
generalRoute.get("/:userId/:type/:id/:option/:page", async (req, res) => {
  ...
  const userLiked = async () => {
    try{
      const docRef = doc(db, `${option === "movie" ? "likedMovies" : "likedSeries"}`, `${userId}`)
      //const likedRef = db.collection(`${option === "movie" ? "likedMovies" : "likedSeries"}/${userId}`);
   ...
   const axiosPromises = [configPromise, userLiked];
